<?php
	...
	if(isset($_POST['table'])) {
		...
		$isPlayer = isset($_POST['Probespiel']) ? 1 : 0;
		$input = 0;
		switch(htmlspecialchars($_POST['table'])) {
			case "member":
				//echo 'POST';
				$input = 1;
				$keys = "memberid,titel,Vorname,Vorname2,Name,Zeichen,Funktion,BegleitpersonVon,strasse,plz,city,land,company,vorwahl,telefon,mobil,email,geburtstag,geschlechtid,firmaid,innungid,Notiz,active,md5hash,isPlayer";
				$values = "NULL,'" . $titel .
				"','" . $Vorname .
				"','" . $Vorname2 .
				"','" . $Name .
