<?php
	...
	$whereStr = $eventid > 0 ? " WHERE a.eventid=" . $eventid : $whereStr;
	$whereStr = $eventid > 0 ? " WHERE a.md5hash='" . $md5hash ."'" : $whereStr;
	
	$event_memberid = 0;

	if($_GET and isset($_GET['meeid'])) {
		$event_memberid = htmlspecialchars($_GET['meeid']);
		$event_memberid = intval($event_memberid);
	}
	$whereStr = $event_memberid > 0 ? " WHERE a.event_memberid=" . $event_memberid : $whereStr;
	$whereStr = $eventid > 0 ? " WHERE e.eventid=" . $eventid : $whereStr;
	...
	while(($rowEventMember = $queryEventMember->fetch_assoc()) !== null) {
		...
		if($eventid === 0) {
			//$eventid = $rowEventMember['eventid'];
		}
		if($event_memberid === 0) {
			$event_memberid = $rowEventMember['event_memberid'];
		}
	}
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-xs-12 col-md-12">
				<div class="list-group">
					...
					<?php if($eventid != 0): ?>
					<a class="list-group-item list-group-item-action" href="checkInn.php?eventid=0">alle Veranstaltungen</a>
					<?php endif; ?>
				</div>
			</div>
		</div>
		<div class="row">
			...
			<form action="form_Submit.php" method="post">
				...
				<input type="hidden" name="isPaid" value="<?php echo $outputEventMember[0]['isPaid'] ?>">
				...
				<!--<div class="row">-->
					...
					<div class="form-group">
						...
						<select name="eventid" id="eventid" class="form-control" onchange="reloadForEvent(this)">
							 ...
							 <?php
							 if($eventid == 0){
							 	echo "<option value=\"0\">Veranstaltung auswählen</option>\n";
							 }
						...
						<tbody>
							 ...
							 foreach($outputEventMember as $elem) {
									...
									if($elem['isPaid'] == 1) {
										...
										echo "<a class='paid' href='paidInn.php?mid=". $elem['memberid'] ."&eventid=".$eventid."'>€</a>";
									}
				...
				<div class="col-xs-12 col-md-12">
					<button type=<?php if($event_memberid === 0) {echo '"button" onclick="goToList(this.form)"'; } else { echo '"submit"';} ?> class="btn btn-primary col-md-6"><?php $aendern = $event_memberid === 0 ?  "aktualisieren" :  "bestätigen"; echo $aendern; ?></button>
				</div>
			</form>
		</div>
		</div>
